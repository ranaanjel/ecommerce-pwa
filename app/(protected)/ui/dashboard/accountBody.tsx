...
export function AccountBody() {
    ...
    useEffect(function () {
        ...
        if (data) {
            let userIdentification = data.user?.id;
            setUserId(userIdentification as string)

            startTransition(async function () {
                let returnData = await InfoValue("restaurant");
                let {name, contact, representatitve} = returnData;
                // console.log(name, contact, representatitve, returnData)
                if(contact == "") {
                    contact = "phone" in data.user! ? data.user.phone : "";
                }
                setRestaurantDetails(() => {
                    return { name, contact, representatitve:returnData.representative }
                })
                setLoading(false);
                let versionData = await InfoValue("version");
...
function RestaurantInfo({ restaurantDetails, userDetails }: { userDetails: { name: string, image: string, id: string }, restaurantDetails: { name: string, contact: string, representatitve: string } }) {
    ...
    return <div className="flex px-8 py-4 gap-4 items-center w-full">
        ...
        <div className="flex flex-col leading-4.5 w-[80%]">
            ...
            <div className="text-md capitalize text-gray-600">
                {restaurantDetails.representatitve}
            </div>
            <div className="text-sm text-gray-500">
                {restaurantDetails.contact || "xxxxx-xxxxx"}
            </div>
        </div>
    </div>
...
function OrderNStatement({ setModalConstruction, setCurrentRow }: { setModalConstruction: React.Dispatch<SetStateAction<boolean>>, setCurrentRow: React.Dispatch<SetStateAction<string>> }) {
    ...
    return <div className="px-8 py-4">
        ...
        <div className="bg-white rounded w-full mt-4 text-gray-700">
            ...
            <ItemRow onclick={function () {
                // setModalConstruction(true)
                //TODO - message to whatsapp
                setCurrentRow("Need help")
            }} icon={<InfoIcon></InfoIcon>} text="Need help" borderUp={true}>
                    <Link className="w-full absolute top-0 left-0 h-full z-10" onClick={function () {
                    console.log("click")
                }} href={"https://wa.me/8287470325?text=Need help!"} ></Link>
            </ItemRow>
        </div>
    </div>
}
function PaymentOptions({ setModalConstruction, setCurrentRow }: { setModalConstruction: React.Dispatch<SetStateAction<boolean>>, setCurrentRow: React.Dispatch<SetStateAction<string>> }) {
    ...
    return <div className="px-8 py-4">
        ...
        <div className="bg-white rounded w-full mt-4 text-gray-700">
            ...
            }} icon={<ContactRound></ContactRound>} text="Contact us" borderUp={true}>
                <Link className="w-full absolute top-0 left-0 h-full z-10" onClick={function () {
                    console.log("click")
                }} href={"tel:8287470325"} ></Link>
            </ItemRow>
        </div>
    </div>
}
function Others({ setModalConstruction, setCurrentRow, userId }: { userId: string, setModalConstruction: React.Dispatch<SetStateAction<boolean>>, setCurrentRow: React.Dispatch<SetStateAction<string>> }) {
    ...
    return <div className="px-8 py-4">
        ...
        <div className="bg-white rounded w-full mt-4 text-gray-700">
            ...
            }} icon={<PackagePlus></PackagePlus>} text="Request new product" borderUp={true}>
                <Link className="w-full absolute top-0 left-0 h-full z-10" onClick={function () {
                    console.log("click")
                }} href={"https://wa.me/8287470325?text=Requesting a new product!"} ></Link>
            </ItemRow>
            {/* <ItemRow onclick={function () {
                //notification page
                setModalConstruction(true)
...
function ItemRow({ icon, text, borderUp, onclick, children }: {children?:React.ReactNode, onclick?: () => void; icon: React.ReactElement, text: string, borderUp: boolean }) {
    let classValue = ""
    if (borderUp) {
